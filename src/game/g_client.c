...
void ClientUserinfoChanged( int clientNum )
{
  ...
  if( client->ps.stats[ STAT_PCLASS ] == PCL_HUMAN && BG_InventoryContainsUpgrade( UP_BATTLESUIT, client->ps.stats ) )
  {
    ...
    //client->ps.stats[ STAT_PCLASS ] = PCL_HUMAN_BSUIT2;
    //client->pers.classSelection = PCL_HUMAN_BSUIT;
  }
  else if ( client->ps.stats[ STAT_PCLASS ] == PCL_HUMAN_BSUIT && !BG_InventoryContainsUpgrade( UP_BATTLESUIT, client->ps.stats ) )
  {
    Com_sprintf( buffer, MAX_QPATH, "%s/%s",  BG_FindModelNameForClass( PCL_HUMAN ),
                                              BG_FindSkinNameForClass( PCL_HUMAN ) );
    //client->ps.stats[ STAT_PCLASS ] = PCL_HUMAN;
    //client->pers.classSelection = PCL_HUMAN;
  }
...
char *ClientConnect( int clientNum, qboolean firstTime, qboolean isbot )
{
  ...
  if( G_FilterPacket( value ) )
    return "Estas censurado en este servidor.";
  ...
  if( firstTime && isbot == qfalse )
    trap_SendServerCommand( -1, va( "print \"%s" S_COLOR_WHITE " conectado\n\"", client->pers.netname ) );
...
#define F_VEL   50.0f
  ...
  if( level.intermissiontime && client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS )
  ...
  // A ver si aca se arregla el problema de que los bots
  // reaparecen muriendo debido a una granada o cualquier
  // cosa que los mata de nuevo en el mismo lugar donde
  // ya habian muerto  
  // 05/01/2010 sigue el problema
...
void ClientDisconnect( int clientNum )
{
  ...
  ent->client->ps.isHumanBot = qfalse; // Dejó de ser un bot
