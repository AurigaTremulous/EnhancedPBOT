...
char *modNames[ ] =
{
  ...
  "MOD_MDRIVER_SPLASH",
  "MOD_DEFENCE_COMPUTER",
  "MOD_DEFENCE_TELENODE",
  "MOD_DEFENCE_MEDISTATION",
  "MOD_DEFENCE_REPEATER"
};
...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath )
{
  ...
  if( level.intermissiontime && self->client->ps.stats[ STAT_PTEAM ] != PTE_ALIENS )
    return;

  self->client->ps.pm_type = PM_DEAD;
  self->suicideTime = 0;
  
  if ( self->client->ps.stats[ STAT_PCLASS ] == PCL_ALIEN_BUILDER0_UPG && random() < 0.45f )
  {
    gentity_t *blast;
    blast = G_Spawn();
    ...
    blast->splashDamage = ABUILDER_UPG_RESTS_DMG;
    blast->splashRadius = ABUILDER_UPG_RESTS_RADIUS;
    blast->methodOfDeath = MOD_ASPAWN;
  }
  ...
  if( !tk )
  {
	...
	if (attacker->client)
	{
		...
		G_TeamCommand( PTE_HUMANS,
    		va( "print \"^2%s^7 ha ^3matado^7 un alien mas.\n\"", attacker->client->pers.netname) );
	    }
	    //G_LogOnlyPrintf("^2%s^7 ^3mata^7 un alien.\n", attacker->client->pers.netname);
	}
	else
	if ( obit == "MOD_MGTURRET" || obit == "MOD_TESLAGEN" )
	{
	    G_TeamCommand( PTE_HUMANS,
    	    va( "print \"^7Nuestras defensas ^3matan^7 otro alien.\n\"") );
	    //G_LogOnlyPrintf("^2Defensas^7 contra otro alien.\n");
	}
	else
	if ( obit == "MOD_REACTOR" )
	{
	    G_TeamCommand( PTE_HUMANS,
    	    va( "print \"^7ATENCION:^7 Defensas del Reactor ^3matan^7 un alien.\n\"") );
	    //G_LogOnlyPrintf("^2Defensas^7 contra otro alien.\n");	
	}
    }
    if ( self->client->pers.teamSelection == PTE_HUMANS )
    {
	if (attacker->client && attacker->client != self->client)
	{
	    G_TeamCommand( PTE_HUMANS,
    	    va( "print \"^2%s^7 ha sido ^1asesinado^7 por un alien.\n\"", self->client->pers.netname) );
	}
	else
	{
	    G_TeamCommand( PTE_HUMANS,
    	    va( "print \"^2%s^7 ha ^1muerto^7.\n\"", self->client->pers.netname) );
	}
	//G_LogOnlyPrintf("^2%s^7 ^1asesinado^7 por un alien.\n", self->client->pers.netname);
    }
  }
  else if( attacker && attacker->client )
  {
	...
	{
	G_TeamCommand( PTE_HUMANS,
        va( "print \"^2%s^7 fue matado por su ^1COMPANYERO^7 %s^7 (Le hizo %d de perjuicio de un maximo de %d)\n\"",
    ...
    G_LogOnlyPrintf("^2%s fue matado por su COMPANYERO %s (Le hizo %d de perjuicio de un maximo de %d)\n",
    self->client->pers.netname, attacker->client->pers.netname, self->client->tkcredits[ attacker->s.number ], self->client->ps.stats[ STAT_MAX_HEALTH ] );
    trap_SendServerCommand( attacker - g_entities,
      va( "cp \"Mataste a \n^2%s^7\n\n^7 quien es tu \n^1COMPANYERO^7.\n\n^3NO LO VUELVAS A HACER!^7\"", self->client->pers.netname ) );
  }
  ...
  if( attacker && attacker->client )
  {
   ...
   if( g_devmapKillerHP.integer && g_cheats.integer ) 
   {
      ...
      if( g_retribution.integer) {
		...
		if( tk_value > 0 ) {
		  ...
		  trap_SendServerCommand( self->client->ps.clientNum,
		    va( "print \"Recibidos ^3%d %s ^7de %s ^7en retribucion.\n\"",
			tk_value, type, attacker->client->pers.netname ) );
		  trap_SendServerCommand( attacker->client->ps.clientNum,
		    va( "print \"Transferidos ^3%d %s ^7a %s ^7en retribucion.\n\"",
    ...
    else
    {
      ...
      {
	    ambush_nexttime = ambush_nexttime - g_ambush.integer*2; // menos tiempo por cada uno que matamos.
              //turn ff off, otherwise all tyrants kill each others without humans
	      if (g_ambush_no_egg_ffoff.integer == 1 && g_ambush_stage.integer == 9)
              {
  ...
  if( totalDamage > 0.0f )
  {
    ...
    {
      // Invasion Alienigena
      // Si un alien muere estando en el maximo stage posible
      // pierde un credito.
      if (g_ambush_stage.integer >= g_ambush_maxstage.integer)
      {
        G_AddCreditToClient( self->client, -1, qtrue );
      }
  ...
  self->client->respawnTime = level.time + 2900;
         ...
         vec3_t dir, vec3_t point, int damage, int dflags, int mod )
{
  ...
  if( level.intermissionQueued && (!client || (client && client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS)) )
    return;
  ...
  if (client && BG_InventoryContainsUpgrade( UP_BATTLESUIT, client->ps.stats ))
  {
    knockback = (int) (damage * BSUIT_K_REDUCTION);
  }
  else
  {
    knockback = damage;
  ...
  //ROTAX
  // Esto hay que hacerlo bien junto con el codigo NO EGGS de g_bot.c
  ...
  if( client )
  {
    ...
    {
	take = (int) (take * BSUIT_DMG_REDUCTION);
    }
...
void ambush_next_stage()
{
  ...
  trap_Cvar_Set("g_ambush_stage", va("%d", g_ambush_stage.integer));

  if (g_ambush_stage_suicide.integer == 1)
    kill_aliens();

  ambush_nexttime = level.time + increment + g_ambush.integer*1000;
  ...
  /*trap_SendServerCommand( -1, va( "cp \"^1--> Aliens Evolved (^2Stage %i^1) <--\n^1(comming in %s)^7\"", ROTACAK_ambush_stage, string ) );
  trap_SendServerCommand( -1, va( "print \"^1--> Aliens Evolved (^2Stage %i^1) <--\n^1(comming in %s)^7\n\"", ROTACAK_ambush_stage, string ) );*/
  G_TeamCommand( PTE_ALIENS,
  va( "print \"^3Evolucion^7 a ^2nivel %i^7 (nacimientos en %s)\n\"", g_ambush_stage.integer, string) );
  G_LogPrintf("Aliens Evolved (Stage %i) (comming in %s)\n", g_ambush_stage.integer, string );
}
