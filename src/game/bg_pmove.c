...
float pm_duckScale = 0.37f;
float pm_swimScale = 0.40f;
float pm_ladderScale = 0.43f;
...
static float PM_CmdScale( usercmd_t *cmd )
{
  ...
  if( pm->ps->stats[ STAT_PTEAM ] == PTE_HUMANS && pm->ps->pm_type == PM_NORMAL )
  {
    ...
    if( pm->ps->stats[ STAT_STATE ] & SS_CREEPSLOWED )
    {
      ...
      // En Invasion Alienigena el battlesuit es un traje pesado y automatizado
      // por lo que apenas le afectan las trampas porque tiene mas fuerza 
      // que velocidad
...
static qboolean PM_CheckJump( void )
{
  ...
  // El battlesuit consume menos stamina al saltar porque es asistido
  if( pm->ps->stats[ STAT_PTEAM ] == PTE_HUMANS )
  {
    if (BG_InventoryContainsUpgrade( UP_BATTLESUIT, pm->ps->stats ) )
	pm->ps->stats[ STAT_STAMINA ] -= 200;
    else
	pm->ps->stats[ STAT_STAMINA ] -= 400;
  }
  ...
  else
  {
	...
	pm->ps->velocity[ 2 ] = BG_FindJumpMagnitudeForClass( pm->ps->stats[ STAT_PCLASS ] );
      	...
      	pm->ps->velocity[ 2 ] = BG_FindJumpMagnitudeForClass( PCL_HUMAN_BSUIT );
  }
...
static void PM_LadderMove( void )
{
  ...
  cubsuit = BG_InventoryContainsUpgrade( UP_BATTLESUIT, pm->ps->stats );

  if (cubsuit)
  {
    scale = PM_CmdScale( &pm->cmd ) * 1.8f;
  }
  else
  {
    scale = PM_CmdScale( &pm->cmd );
  ...
  if (cubsuit)
  {
     if( wishspeed > pm->ps->speed * pm_ladderScale * 1.8f )
	wishspeed = pm->ps->speed * pm_ladderScale * 1.8f;  
  }
  else
  {
...
/*  else
    BG_FindViewheightForClass( PCL_HUMAN_BSUIT, &PCvh, &PCcvh );*/
...
static void PM_Weapon( void )
{
  ...
  if( pm->ps->weaponstate == WEAPON_RELOADING )
  {
    ...
    {
        if( BG_InventoryContainsUpgrade( UP_BATTPACK, pm->ps->stats ) )
    	    ammo = (int)( (float)ammo * BATTPACK_MODIFIER );

	// In Invasion the Battlesuit have an onboard battpack :P
        if (BG_InventoryContainsUpgrade( UP_BATTLESUIT, pm->ps->stats ) )
    	    ammo = (int)( (float)ammo * BSUIT_BATTPACK_MODIFIER );
    }
    else
    {
	// In Invasion the Battlesuit have an ammo chain addition for more ammo in weapons
        if (BG_InventoryContainsUpgrade( UP_BATTLESUIT, pm->ps->stats ))
	{
	    if (BG_InventoryContainsWeapon( WP_MACHINEGUN, pm->ps->stats ) )
		ammo += BSUIT_AMMOCHAIN_RIFLE;
	    else
	    if (BG_InventoryContainsWeapon( WP_CHAINGUN, pm->ps->stats ) )
		ammo += BSUIT_AMMOCHAIN;
	}
  ...
  switch( pm->ps->weapon )
  {
    ...
    case WP_LUCIFER_CANNON:
      ...
      {
        if( pm->ps->stats[ STAT_MISC ] < LCANNON_TOTAL_SPEED )
        {
          pm->ps->weaponTime = 0;
          pm->ps->weaponstate = WEAPON_READY;
          return;
      ...
      if( pm->ps->stats[ STAT_MISC ] > LCANNON_MIN_SPEED )
        attack1 = !attack1;
  ...
  if( pm->ps->weapon == WP_CHAINGUN )
  {
    if( pm->ps->pm_flags & PMF_DUCKED ||
        BG_InventoryContainsUpgrade( UP_BATTLESUIT, pm->ps->stats ) )
    {
      pm->ps->delta_angles[ PITCH ] -= ANGLE2SHORT( ( ( random() * 0.5f ) - 0.125f ) * ( 30.0f / (float)addTime ) );
      pm->ps->delta_angles[ YAW ] -= ANGLE2SHORT( ( ( random() * 0.5f ) - 0.25f ) * ( 30.0f / (float)addTime ) );
    }
    else
    {
      pm->ps->delta_angles[ PITCH ] = pm->ps->delta_angles[ PITCH ] - 
      ANGLE2SHORT( ( ( random() * 8.0f ) - 2.0f ) * ( 30.0f / (float)addTime ) );
      
      pm->ps->delta_angles[ YAW ] = pm->ps->delta_angles[ YAW ] - 
      ANGLE2SHORT( ( ( random() * 8.0f ) - 4.0f ) * ( 30.0f / (float)addTime ) );
      //pm->ps->delta_angles[ YAW ] -= 2000;//ANGLE2SHORT( ( ( random() * 8.0f ) - 4.0f ) * ( 30.0f / (float)addTime ) );
    }
  }
