...
vmCvar_t  g_RandomStage;
vmCvar_t  g_RandomStageHumans;
vmCvar_t  g_RandomStageAliens;
vmCvar_t  g_RandomAmbush;
vmCvar_t  g_RandomDefensesHumans;
vmCvar_t  g_RandomDefensesAliens;
vmCvar_t  g_RandomBaseHumans;
vmCvar_t  g_RandomBaseAliens;

int  ROTACAK_ambush_rebuild_time_temp = 0;
int  ambush_nexttime = 0;
...
static cvarTable_t   gameCvarTable[ ] =
{
  ...
  { &g_speed, "g_speed", "220", CVAR_SERVERINFO, 0, qtrue  }, // 320
  ...
  { &g_builder_range, "g_builder_range", "160", CVAR_ARCHIVE, 0, qfalse },  
  ...
  { &g_bot_grenade, "g_bot_grenade", "1", CVAR_ARCHIVE, 0, qfalse },
...
//  { &g_ambush_tyrants_to_win, "g_ambush_tyrants_to_win", "20", CVAR_SERVERINFO | CVAR_ARCHIVE, 0, qfalse  },
  ...
  { &g_ambush_no_egg_ffoff, "g_ambush_no_egg_ffoff", "0", CVAR_ARCHIVE, 0, qfalse  },
  ...
  { &g_ambush, "g_ambush", "200", CVAR_SERVERINFO | CVAR_ARCHIVE, 0, qfalse  },
  { &g_ambush_stage, "g_ambush_stage", "0", CVAR_SERVERINFO | CVAR_ARCHIVE, 0, qfalse  },
  { &g_ambush_maxstage, "g_ambush_maxstage", "8", CVAR_SERVERINFO, 0, qfalse  },
  { &g_minPlayers, "g_minPlayers", "4", CVAR_SERVERINFO, 0, qfalse  },

  { &g_RandomStage, "g_RandomStage", "0", CVAR_ARCHIVE, 0, qfalse  },
  { &g_RandomStageHumans, "g_RandomStageHumans", "0", CVAR_ARCHIVE, 0, qfalse  },
  { &g_RandomStageAliens, "g_RandomStageAliens", "0", CVAR_ARCHIVE, 0, qfalse  },
  { &g_RandomAmbush, "g_RandomAmbush", "0", CVAR_ARCHIVE, 0, qfalse  },
//  { &g_RandomDefense, "g_RandomDefense", "0", CVAR_SERVERINFO, 0, qfalse  },
  { &g_RandomDefensesHumans, "g_RandomDefensesHumans", "0", CVAR_ARCHIVE, 0, qfalse  },
  { &g_RandomDefensesAliens, "g_RandomDefensesAliens", "0", CVAR_ARCHIVE, 0, qfalse  },
//  { &g_RandomBase, "g_RandomBase", "0", CVAR_SERVERINFO, 0, qfalse  },
  { &g_RandomBaseHumans, "g_RandomBaseHumans", "0", CVAR_ARCHIVE, 0, qfalse  },
  { &g_RandomBaseAliens, "g_RandomBaseAliens", "0",  CVAR_ARCHIVE, 0, qfalse  }
};
                              ...
                              int arg10, int arg11 )
{
  switch( command )
  {
    ...
    case GAME_CLIENT_CONNECT:
      return (intptr_t)ClientConnect( arg0, arg1, qfalse );
...
void G_InitGame( int levelTime, int randomSeed, int restart )
{
  ...
  //ROTAX
  trap_Cvar_Set( "g_ambush", "200" );
  trap_Cvar_Set( "g_ambush_stage", "0" );
  trap_Cvar_Set( "g_friendlyFire", "1" );
  trap_Cvar_Set( "g_friendlyFireAliens", "1" );

  // Random initializations
  if (g_RandomStage.integer)
  {
	arandom_stage = hrandom_stage = rand() % g_RandomStage.integer +1;
  } 
  else
  {
	  arandom_stage = rand() % g_RandomStageAliens.integer +1;
	  hrandom_stage = rand() % g_RandomStageHumans.integer +1;
  } 

	/*if (arandom_stage > g_alienStage.integer)
	{
		trap_Cvar_Set( "g_alienStage", va( "%d", arandom_stage ) );
	}
	if (hrandom_stage > g_humanStage.integer)
	{
		trap_Cvar_Set( "g_humanStage", va( "%d", hrandom_stage ) );
	}*/

  // Random ambush stage
  if (g_RandomAmbush.integer > 0)
  {
	  ambush_random = rand() % g_RandomAmbush.integer +1;
	  trap_Cvar_Set( "g_ambush_stage", va( "%d", ambush_random ) );
  }


  ROTACAK_ambush_rebuild_time_temp = 0;
  ambush_nexttime = level.time + g_ambush.integer*1000;
...
void MoveClientToIntermission( gentity_t *ent )
{
  ...
  //if(ent->r.svFlags & SVF_BOT) {ClientDisconnect(ent->client->ps.clientNum);}
...
void BeginIntermission( void )
{
  ...
  {
    client = g_entities + i;
    // Los aliens siguen jugando siempre
...
void ExitLevel( void )
{
  ...
  while( ( tmp = level.buildHistory ) )
  {
    ...
    // Aliens does not win, then continue...
    ...
    // Aliens win, retry because humans will win
    else
    {
        trap_SendConsoleCommand( EXEC_APPEND, "map_restart\n" );
    }
...
void CheckIntermissionExit( void )
{
  ...
  for( i = 0; i < g_maxclients.integer; i++ )
  {
    ...
    // Los aliens no cuentan en esto
    if( cl->ps.stats[ STAT_PTEAM ] == PTE_ALIENS )
      continue;

    // Los bots tampoco
    // bueno, no se pude detectar la bandera. Asi que
    // en vez de eso los bots activan el ready de una vez
...
void CheckExitRules( void )
{
  ...
  if( g_timelimit.integer )
  {
    if( level.time - level.startTime >= g_timelimit.integer * 60000 )
    {
      ...
      trap_SendServerCommand( -1, "print \"Se Termino el tiempo\n\"" );
    ...
    {
      trap_SendServerCommand( -1, "cp \"quedan solo 5 minutos!\"" );
      level.timelimitWarning = TW_IMMINENT;
    }
    else if( level.time - level.startTime >= ( g_timelimit.integer - 1 ) * 60000 &&
          level.timelimitWarning < TW_PASSED )
    {
      trap_SendServerCommand( -1, "cp \"queda solo 1 minuto!\"" );
      level.timelimitWarning = TW_PASSED;
    }
  }
        ...
        ( level.numLiveAlienClients == 0 ) ) )
  {
    ...
    //ROTAX
    //if (ROTACAK_ambush_stage < g_ambush_maxstage.integer)
    {
      Com_sprintf( string, sizeof( string ), "%i:%i%i", min, tens, sec );
      
      if (g_ambush_stage_suicide.integer == 1)
        trap_SendServerCommand( -1, va("print \"^3Mision Completada^7 (^2Record: %s^7) (Suicidio: Activado)\n\"", string));
      else
        trap_SendServerCommand( -1, va("print \"^3Mision Completada^7 (^2Record: %s^7) (Suicidio: Desactivado)\n\"", string));
    }
    //else
    //trap_SendServerCommand( -1, "print \"Mission complete\n\"");
      
    trap_SetConfigstring( CS_WINNER, "Mision Completada" );
    LogExit( "Humans win." );
             ...
             ( level.numLiveHumanClients == 0 ) ) )
  {
    ...
    Com_sprintf( string, sizeof( string ), "%i", min );
    trap_SendServerCommand( -1, va("print \"^1Mision Fracasada en %s minutos^7\n\"", string));
    trap_SetConfigstring( CS_WINNER, va("Mision Fracasada en %s minutos", string));
    LogExit( "Aliens win." );
...
void CheckVote( void )
{
  ...
  if( level.voteExecuteTime && level.voteExecuteTime < level.time )
  {
    ...
    {
	if ( !Q_stricmp( level.voteString, "map *" ) )
	{
	    level.restarted = qtrue;
	    ExitLevel( );
	}
...
void CheckCountdown( void )
{
  ...
  lastmsg = level.time;
  if( timeleft > 0 )
    trap_SendServerCommand( -1, va( "cp \"^1Espera:^7\n^%i----- ^7%i ^%i-----\"", timeleft % 7, timeleft, timeleft % 7 ) );
  else if( timeleft == 0 ) 
    trap_SendServerCommand( -1, "cp \"^2----- YA! -----^7\"" );
}
