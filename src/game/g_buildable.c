  ...
  if( attacker && attacker->client )
  {
    ...
    else
    {
      G_TeamCommand( PTE_ALIENS,
        va( "print \"%s ^6DESTRUIDO^7 por el companyero %s^7\n\"",
          BG_FindHumanNameForBuildable( self->s.modelindex ), 
          attacker->client->pers.netname ) );
      G_LogOnlyPrintf("%s ^6DESTRUIDO^7 por el companyero %s^7\n",
          BG_FindHumanNameForBuildable( self->s.modelindex ), 
          attacker->client->pers.netname ); 
    }
  ...
  if( self->spawned )
  {
		 ...
		 ent->client->ps.stats[ STAT_PTEAM ] == PTE_ALIENS )
	{
	    ...
	    //five seconds of blocked by client and...
	  {
	    ...
	    trap_SendServerCommand( ent-g_entities, "cp \"No bloquees el nacimiento!\"" );
	  }
...
It is for alien builds with thorns
When an human touch it, it will receive damage (because the
thorns are dangerous :P)
================
*/
void AThorns_Touch( gentity_t *self, gentity_t *other, trace_t *trace )
{
  ...
  // Don't hurt aliens, they know how to move safe between thorns
  if( client && client->ps.stats[ STAT_PTEAM ] == PTE_ALIENS )
  {
    // A granger bot will be hidden there :P
    if ( (other->r.svFlags & SVF_BOT) &&
	 self->s.modelindex == BA_A_HOVEL &&
	 (client->ps.stats[ STAT_PCLASS ] == PCL_ALIEN_BUILDER0 || 
         client->ps.stats[ STAT_PCLASS ] == PCL_ALIEN_BUILDER0_UPG) )
    {
  ...
  // Do the damage
  // TODO: Knock the player out so it don't continue getting damage
  if ( other->takedamage )
  {
    G_Damage( other, self, self, NULL, NULL, THORNS_DMG, 0, MOD_ASPAWN );
  }
}
...
qboolean ATrapper_CheckTarget( gentity_t *self, gentity_t *target, int range )
{
  ...
  // Modified for Invasion: Now a big field of vision ;)
...
void HReactor_Think( gentity_t *self )
{
  ...
  //int	    attack_dmg = REACTOR_ATTACK_DAMAGE;
  ...
  /*if ( G_FindDCC(self) )
  {
    attack_dmg = (int)(attack_dmg * REACTOR_ATTACK_DC_UP);
  }*/
...
void HMedistat_Think( gentity_t *self )
{
  ...
  if( self->spawned )
  {
    ...
    else if( self->enemy ) //heal!
    {
      ...
      // No no, que nos de un medkit si o si porque no importa si estamos curados o no
      // nos lo puede dar igual :P
  ...
  if( self->spawned )
  {
		 ...
		 ent->client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS )
	{
	    ...
	    //five seconds of blocked by client and...
	  {
	    ...
	    VectorAdd( ent->client->ps.velocity, velocity, ent->client->ps.velocity );
	    trap_SendServerCommand( ent-g_entities, "cp \"No obstruyas el telenodo!\"" );
	  }
	  else if( !self->spawnBlockTime )
	    self->spawnBlockTime = level.time;
        }
...
qboolean PlaceForTelenode( gentity_t *built, vec3_t rand_origin, vec3_t basemins, vec3_t basemaxs )
{
  vec3_t	dest, floor_origin, mins, maxs;
  trace_t	tr, tr1;
  int a;
	
		// Cabe un telenodo acá?
		BG_FindBBoxForBuildable( BG_FindBuildNumForName("telenode"), mins, maxs );
		VectorSet(mins, -41, -41, 10);
		VectorSet(maxs, 41, 41, 10);
		VectorAdd(basemins, mins, mins);
		VectorAdd(basemaxs, maxs, maxs);
		for (a = 0; a<360; a=a+1)
		{
			new_origin[0] = rand_origin[0]+cos(a)*100;
			new_origin[1] = rand_origin[1]+sin(a)*100;
			new_origin[2] = rand_origin[2] + 12; // - 12;
			// Buscamos un lugar donde halla espacio para ponerlo
			VectorSet( floor_origin, new_origin[0], new_origin[1], new_origin[2]-50);
			VectorScale( new_origin, -4096.0f, dest );
			VectorAdd( dest, new_origin, dest );
			trap_Trace( &tr, new_origin, mins, maxs, dest, (built - g_entities), MASK_PLAYERSOLID );
			trap_Trace( &tr1, new_origin, mins, maxs, floor_origin, (built - g_entities), MASK_PLAYERSOLID );
			if ( !(tr.allsolid) && !(tr.startsolid) && (tr1.fraction < 1.0f) )
			{
				return qtrue;
			}
		}
...
qboolean PlaceForTurret( gentity_t *built, vec3_t rand_origin, vec3_t basemins, vec3_t basemaxs, int i )
{
  vec3_t	dest, floor_origin, mins, maxs;
  trace_t	tr, tr1;
  int a;
	  
			BG_FindBBoxForBuildable( BG_FindBuildNumForName("mgturret"), mins, maxs );
			for (a = 0; a<360; a=a+i*5)
			{
				new_origin[0] = rand_origin[0]+cos(a)*220;
				new_origin[1] = rand_origin[1]+sin(a)*220;
				new_origin[2] = rand_origin[2]+4;
				// Buscamos un lugar donde halla espacio para ponerlo
				VectorSet( floor_origin, new_origin[0], new_origin[1], new_origin[2]-100);
				VectorScale( new_origin, -4096.0f, dest );
				VectorAdd( dest, new_origin, dest );
				trap_Trace( &tr, new_origin, mins, maxs, dest, (built - g_entities), MASK_PLAYERSOLID );
				trap_Trace( &tr1, new_origin, mins, maxs, floor_origin, (built - g_entities), MASK_PLAYERSOLID );
...
qboolean PlaceForEgg( gentity_t *built, vec3_t rand_origin, vec3_t basemins, vec3_t basemaxs )
{
  vec3_t	dest, floor_origin, mins, maxs;
  trace_t	tr, tr1;
  int a;
		...
		// Cabe un egg acá?
		BG_FindBBoxForBuildable( BG_FindBuildNumForName("egg"), mins, maxs );
		for (a = 0; a<360; a=a+1)
		{
			new_origin[0] = rand_origin[0]+cos(a)*100;
			new_origin[1] = rand_origin[1]+sin(a)*100;
			new_origin[2] = rand_origin[2] + 8;
			// Buscamos un lugar donde halla espacio para ponerlo
			VectorSet( floor_origin, new_origin[0], new_origin[1], new_origin[2]-100);
			VectorScale( new_origin, -4096.0f, dest );
			VectorAdd( dest, new_origin, dest );
...
qboolean PlaceForATube( gentity_t *built, vec3_t rand_origin, vec3_t basemins, vec3_t basemaxs )
{
  vec3_t	dest, floor_origin, mins, maxs;
  trace_t	tr, tr1;
  int a;
  
	BG_FindBBoxForBuildable( BG_FindBuildNumForName("acid_tube"), mins, maxs );
	for (a = 0; a<360; a=a+10)
	{
		new_origin[0] = rand_origin[0]+cos(a)*200;
		new_origin[1] = rand_origin[1]+sin(a)*200;
		new_origin[2] = rand_origin[2]+4;
		// Buscamos un lugar donde halla espacio para ponerlo
		VectorSet( floor_origin, new_origin[0], new_origin[1], new_origin[2]-100);
		VectorScale( new_origin, -4096.0f, dest );
...
================
*/
vec3_t humans_rand_origin;
qboolean humans_rand_origin_isset = qfalse;
vec3_t aliens_rand_origin;
qboolean aliens_rand_origin_isset = qfalse;
static gentity_t *G_Build( gentity_t *builder, buildable_t buildable, vec3_t origin, vec3_t angles )
{
  gentity_t *built;
  buildHistory_t *new;
  vec3_t    normal;
  vec3_t	rand_origin, dest, floor_origin, basemins, basemaxs, mins, maxs;
  double randombase;
  trace_t	tr, tr1;
  qboolean delpath = qfalse;
  int i, i2, a;
  int iinc, iini;
  //buildable_t new_buildable;
  qboolean isset = qtrue;
  ...
  if( !builder->client )
  {
    ...
    // Posicionamiento aleatorio (random) de las bases
    ...
    if (BG_FindTeamForBuildable(buildable) == BIT_HUMANS)
    {
		iinc = 1;
		iini = 0;
		if (humans_rand_origin_isset == qfalse && (g_RandomBaseHumans.integer > 0))
		{
			BG_FindBBoxForBuildable( BG_FindBuildNumForName("reactor"), basemins, basemaxs );
			randombase = (double)g_RandomBaseHumans.integer/100;
			isset = qfalse;
			trap_Cvar_Set( "g_RandomBaseHumans", "0" );
		}
		else
		{
			VectorCopy( humans_rand_origin, rand_origin );
		}
	}

    if (BG_FindTeamForBuildable(buildable) == BIT_ALIENS)
    {
		iinc = -1;
		iini = level.numPaths-1;
		if (aliens_rand_origin_isset == qfalse && (g_RandomBaseAliens.integer > 0))
		{
			BG_FindBBoxForBuildable( BG_FindBuildNumForName("overmind"), basemins, basemaxs );
			randombase = (double)g_RandomBaseAliens.integer/100;
			isset = qfalse;
			trap_Cvar_Set( "g_RandomBaseAliens", "0" );
		}
		else
		{
			VectorCopy( aliens_rand_origin, rand_origin );
		}
	}
    
    /* Busca un nuevo lugar para la base
     * Para eso utilizo los paths del mapa.
     * Si el mapa no los tiene, no se va a poder hacer.
     * 
     * Una vez que encuentra un nuevo lugar para la base
     * mueve todo hacia ese sitio. */
    if ((randombase != 0) && (isset == qfalse))
    {
		VectorSet(mins, -180, -180, 0);
		VectorSet(maxs, 180, 180, 0);
		VectorAdd(basemins, mins, mins);
		VectorAdd(basemaxs, maxs, maxs);
		for(i = iini; ((BG_FindTeamForBuildable(buildable) == BIT_HUMANS) && i < level.numPaths) || ((BG_FindTeamForBuildable(buildable) == BIT_ALIENS) && i >= 0); i=i+iinc)
		{
			// Está borrado? No lo usamos entonces.
			for (i2 = 0; i2 < 5; i2++)
			{
			...
			VectorCopy( level.paths[i].coord, rand_origin );
			// Buscamos un lugar donde halla espacio para poner la base
			VectorSet( floor_origin, rand_origin[0], rand_origin[1], rand_origin[2]-50);
			VectorScale( rand_origin, -4096.0f, dest );
			VectorAdd( dest, rand_origin, dest );
			trap_Trace( &tr, rand_origin, basemins, basemaxs, dest, (built - g_entities), MASK_PLAYERSOLID );
			trap_Trace( &tr1, rand_origin, mins, maxs, floor_origin, (built - g_entities), MASK_PLAYERSOLID );
			if ( !(tr.allsolid) && !(tr.startsolid) && (tr1.fraction < 1.0f) && (random() < randombase) )
			{
				/* DEBUG */
				/*trap_SendServerCommand(-1, va("print \"%s: %f, %f, %f\n\"", 
				built->classname,
				origin[0], origin[1], origin[2]));*/

				//VectorCopy( rand_origin, origin );
				/* DEBUG */
				/*trap_SendServerCommand(-1, va("print \"%s: %f, %f, %f\n\"", 
				built->classname,
				rand_origin[0], rand_origin[1], rand_origin[2]));*/
				/* DEBUG */
				/*trap_SendServerCommand(-1, va("print \"%s: %f, %f, %f\n\"", 
				built->classname,
				origin[0], origin[1], origin[2]));*/
				...
				if (BG_FindTeamForBuildable(buildable) == BIT_HUMANS)
				{
					if (PlaceForTelenode( built, rand_origin, basemins, basemaxs ) == qtrue)
					{
						VectorCopy( rand_origin, humans_rand_origin );
						humans_rand_origin_isset = qtrue;
						break;
					}
				}
				...
				if (BG_FindTeamForBuildable(buildable) == BIT_ALIENS)
				{
					if (PlaceForEgg( built, rand_origin, basemins, basemaxs ) == qtrue)
					{					
						VectorCopy( rand_origin, aliens_rand_origin );
						aliens_rand_origin_isset = qtrue;
						break;
					}
				}
			}
		...
		/* Posiciona en el lugar nuevo las entidades */
		
		/* Para Humanos */
		if ((g_RandomBaseHumans.integer > 0) &&
			(BG_FindTeamForBuildable(buildable) == BIT_HUMANS) &&
			(humans_rand_origin_isset == qtrue))
		{
			if (!strcmp(built->classname, "team_human_reactor"))
			{
				VectorCopy( humans_rand_origin, origin );
				VectorCopy( origin, rand_origin );
				/* DEBUG */
				/*trap_SendServerCommand(-1, va("print \"%s: %f, %f, %f\n\"", 
				built->classname,
				origin[0], origin[1], origin[2]));*/
				BG_FindBBoxForBuildable( BG_FindBuildNumForName("mgturret"), mins, maxs );
				for (i=1; i<=g_RandomDefensesHumans.integer; i++)
						for (a = 0; a<360; a=a+i*5)
						{
							new_origin[0] = rand_origin[0]+cos(a)*220;
							new_origin[1] = rand_origin[1]+sin(a)*220;
							...
							if ( !(tr.allsolid) && !(tr.startsolid) && (tr1.fraction < 1.0f) )
							{
								...
								G_Build(builder, BG_FindBuildNumForName("mgturret"), new_origin, angles);
								break;
							}
						}
			}
			else
			if (!strcmp(built->classname, "team_human_spawn"))
			{
			...
			if (!strcmp(built->classname, "team_human_mgturret"))
			{
				/* DEBUG */
				/*trap_SendServerCommand(-1, va("print \"%s: %f, %f, %f\n\"", 
				built->classname,
				origin[0], origin[1], origin[2]));*/
				found = qfalse;
				if (PlaceForTurret(built, rand_origin, basemins, basemaxs, 1) == qtrue )
				{
					VectorCopy( new_origin, origin );
					found = qtrue;
				}
			...
			{
				// El resto de lo que sobra
						BG_FindBBoxForBuildable( buildable, mins, maxs );
						found = qfalse;
						for (a = 0; a<360; a=a+10)
						{
							new_origin[0] = rand_origin[0]+cos(a)*170;
							new_origin[1] = rand_origin[1]+sin(a)*170;
							new_origin[2] = rand_origin[2]+10;
							// Buscamos un lugar donde halla espacio para ponerlo
							VectorSet( floor_origin, new_origin[0], new_origin[1], new_origin[2]-100);
							VectorScale( new_origin, -4096.0f, dest );
							VectorAdd( dest, new_origin, dest );
		...
		/* Para Aliens */
		if ((g_RandomBaseAliens.integer > 0) &&
			(BG_FindTeamForBuildable(buildable) == BIT_ALIENS) &&
			(aliens_rand_origin_isset == qtrue))
		{
			if (!strcmp(built->classname, "team_alien_overmind"))
			{
				VectorCopy( aliens_rand_origin, origin );
				VectorCopy( origin, rand_origin );
				BG_FindBBoxForBuildable( BG_FindBuildNumForName("acid_tube"), mins, maxs );
				for (i=1; i<=g_RandomDefensesAliens.integer; i++)
						for (a = 0; a<360; a=a+i*5)
						{
							new_origin[0] = rand_origin[0]+cos(a)*200;
							new_origin[1] = rand_origin[1]+sin(a)*200;
							...
							if ( !(tr.allsolid) && !(tr.startsolid) && (tr1.fraction < 1.0f) )
							{
								G_Build(builder, BG_FindBuildNumForName("acid_tube"), new_origin, angles);
								break;
							}
						}
			}
			else
			if (!strcmp(built->classname, "team_alien_spawn"))
			{
				found = qfalse;
				if (PlaceForEgg(built, rand_origin, basemins, basemaxs) == qtrue )
				{
			...
			if (!strcmp(built->classname, "team_alien_acid_tube"))
			{
				found = qfalse;
				if (PlaceForATube(built, rand_origin, basemins, basemaxs) == qtrue )
				{
					VectorCopy( new_origin, origin );
					found = qtrue;
				}				
			...
			else
			{
				// El resto de lo que sobra
						...
						for (a = 0; a<360; a=a+8)
						{
							new_origin[0] = rand_origin[0]+cos(a)*150;
							new_origin[1] = rand_origin[1]+sin(a)*150;
							new_origin[2] = rand_origin[2]+8;
							// Buscamos un lugar donde halla espacio para ponerlo
							VectorSet( floor_origin, new_origin[0], new_origin[1], new_origin[2]-100);
							VectorScale( new_origin, -4096.0f, dest );
  ...
  if( builder->client ) {
    if ( G_IsDCCBuilt() == qtrue )
    {
    G_TeamCommand( builder->client->pers.teamSelection,
      va( "print \"Un %s esta ^5siendo construido^7 por %s^7\n\"",
  ...
  /* Borro aquella entidad que no pudo ser posicionada */
  if (!builder->client )
  {
  	if ( found == qfalse )
	{
		built->think = G_FreeEntity;
	}
  }
