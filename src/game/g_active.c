...
void ClientTimerActions( gentity_t *ent, int msec )
{
  ...
  while ( client->time100 >= 100 )
  {
	...
	{
	    if (ent->health <= 40 && BG_InventoryContainsUpgrade(UP_MEDKIT, client->ps.stats))
	    {
		BG_ActivateUpgrade( UP_MEDKIT, client->ps.stats );
	    }
	}*/
    ...
    if( client->ps.weapon == WP_LUCIFER_CANNON )
    {
      ...
      if( client->ps.stats[ STAT_MISC ] < LCANNON_TOTAL_SPEED && ucmd->buttons & BUTTON_ATTACK )
        client->ps.stats[ STAT_MISC ] += (int) ( LCANNON_SECONDARY_SPEED / LCANNON_REPEAT * LCANNON_CHARGEREPEAT );

      if( client->ps.stats[ STAT_MISC ] > LCANNON_TOTAL_SPEED )
        client->ps.stats[ STAT_MISC ] = LCANNON_TOTAL_SPEED;
...
//    G_Printf("serverTime >>>>>\n" );
  ...
  //
  if( level.intermissiontime && ent->client->ps.stats[ STAT_PTEAM ] != PTE_ALIENS )
  {
    ClientIntermissionThink( client );
    // Los bots se ponen automaticamente listos para salir
    if( ent->r.svFlags & SVF_BOT )
    {
	ent->client->readyToExit = 1;
    }
    return;
  }
  ...
  if (BG_InventoryContainsUpgrade(UP_BATTLESUIT, client->ps.stats))
  {
    client->ps.speed = g_speed.value * BG_FindSpeedForClass( client->ps.stats[ STAT_PCLASS ] ) * BSUIT_SPEEDMOD;
  }
  else
  {
