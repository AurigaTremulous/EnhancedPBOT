...
void Cmd_Give_f( gentity_t *ent )
{
  ...
  if( give_all || Q_stricmp( name, "ammo" ) == 0 )
  {
    ...
    if( BG_FindUsesEnergyForWeapon( client->ps.weapon ) )
    {
        ...
        // El battle suit en Invasion tiene un battpack onboard :P
    ...
    else
    {
        ...
        if (BG_InventoryContainsUpgrade( UP_BATTLESUIT, client->ps.stats ))
        {
            ...
            if (BG_InventoryContainsWeapon( WP_CHAINGUN, client->ps.stats ) )
...
void Cmd_Team_f( gentity_t *ent )
{
  ...
  {
    trap_SendServerCommand( ent - g_entities, va( "print \"team: no puedes"
      " entrar (faltan %d segundos para empezar)\n\"",
  ...
  else if( !Q_stricmp( s, "aliens" ) )
  {
    ...
    // En Invasion Alienigena se bloquean los Aliens
    // solo si g_pathediting no esta activada.
    if (g_pathediting.integer <= 0)
	level.alienTeamLocked = qtrue;
    if( level.alienTeamLocked && !force )
    {
      trap_SendServerCommand( ent-g_entities,
        va( "print \"Entrar como alien ha sido ^1BLOQUEADO\n\"" ) );
      return; 
    }
      ...
      ent->client->ps.stats[ STAT_MISC ] > 0 )
  {
   ...
   if (team != PTE_NONE)
   {
 	  ...
 	  {
 		  trap_SendServerCommand( ent-g_entities, va( "print \"Por favor colocate un nombre. Apreta ESC y ve a Options / Game menu o usa el comando /name en la consola\n\"") );
 		  return;
 	  }
   }
  ...
  G_ChangeTeam( ent, team );
   ...
   if( team == PTE_ALIENS ) {
     if ( oldteam == PTE_HUMANS )
       Com_sprintf( buf, sizeof( buf ), "%s^7 muta siendo humano y ahora es un alien.", ent->client->pers.netname );
     else
       Com_sprintf( buf, sizeof( buf ), "%s^7 es un alien mas.", ent->client->pers.netname );
   }
   else if( team == PTE_HUMANS ) {
     if ( oldteam == PTE_ALIENS )
       Com_sprintf( buf, sizeof( buf ), "%s^7 fue curado y convertido a humano.", ent->client->pers.netname );
     else
       Com_sprintf( buf, sizeof( buf ), "%s^7 se une a las fuerzas humanas.", ent->client->pers.netname );
   }
   else if( team == PTE_NONE ) {
     if ( oldteam == PTE_HUMANS )
       Com_sprintf( buf, sizeof( buf ), "%s^7 abandona las fuerzas humanas.", ent->client->pers.netname );
     else
       Com_sprintf( buf, sizeof( buf ), "%s^7 deja el mundo alienigena.", ent->client->pers.netname );
   }
...
void Cmd_CallVote_f( gentity_t *ent )
{
  ...
  else if( !Q_stricmp( arg1, "map" ) )
  {
	...
	Com_sprintf( level.voteDisplayString,
        sizeof( level.voteDisplayString ), "Change to nextmap in maprotation" );
	level.votePassThreshold = g_mapVotesPercent.integer;	
    }
    else
    {
  ...
  else if( !Q_stricmp( arg1, "nextmap" ) )
  {
    if( !Q_stricmp( arg2, "" ) )
    {
        Com_sprintf( level.voteString, sizeof( level.voteString ), "%s %s", "map", "*" );
	Com_sprintf( level.voteDisplayString,
        sizeof( level.voteDisplayString ), "Change to nextmap in maprotation" );
	level.votePassThreshold = g_mapVotesPercent.integer;	
    }
...
void Cmd_CallTeamVote_f( gentity_t *ent )
{
  ...
  else if( !Q_stricmp( arg1, "denybuild" ) )
  {
    ...
    {
      trap_SendServerCommand( ent-g_entities,
        "print \"callteamvote: el jugador ya no tiene derechos de construccion\n\"" );
      return;
    }
    ...
    Com_sprintf( level.teamVoteDisplayString[ cs_offset ],
        sizeof( level.teamVoteDisplayString[ cs_offset ] ),
        "Se le quitan los derechos de construccion desde '%s'", name );
  }
  else if( !Q_stricmp( arg1, "allowbuild" ) )
  {
    ...
    {
      trap_SendServerCommand( ent-g_entities,
        "print \"callteamvote: el jugador ya tiene derechos de construccion\n\"" );
      return;
    }
...
=================
*/
extern int Bot_GetAmbushAlienClass( void );
void Cmd_Class_f( gentity_t *ent )
{
  ...
  if( ent->client->sess.sessionTeam == TEAM_SPECTATOR )
  {
    ...
    if( ent->client->pers.teamSelection == PTE_ALIENS )
    {
      ...
      {
        trap_SendServerCommand( ent-g_entities,
          va( "print \"No puedes aparecer con la clase %s\n\"", s ) );
        return;
      } 
      ...
      {
        trap_SendServerCommand( ent-g_entities,
          va( "print \"La clase %s no esta permitida\n\"", s ) );
        return;
      }
      ...
      {
        trap_SendServerCommand( ent-g_entities, "print \"Tus derechos de construccion han sido revocados\n\"" );
        return;
      }	
	...
	{
	    if (random() < 0.50f)
		newClass = Bot_GetAmbushAlienClass();
	}
...
void Cmd_Destroy_f( gentity_t *ent )
{
  ...
  {
    trap_SendServerCommand( ent-g_entities,
      "print \"Tus derechos de construccion han sido revocados\n\"" );
    return;
  }
          ...
          ( ent->client->ps.weapon <= WP_HBUILD ) ) )
    {
	  ...
	  {		  
            ...
            G_TeamCommand( ent->client->pers.teamSelection,
              va( "print \"%s ^6DESARMADO^7 por %s^7\n\"",
                BG_FindHumanNameForBuildable( traceEnt->s.modelindex ), 
                ent->client->pers.netname ) );

            G_LogPrintf( "Decon: %i %i 0: %s^7 desarmado %s\n",
...
void Cmd_ActivateItem_f( gentity_t *ent )
{
  ...
  if( upgrade != UP_NONE && BG_InventoryContainsUpgrade( upgrade, ent->client->ps.stats ) )
    ...
    trap_SendServerCommand( ent-g_entities, va( "print \"No tienes %s\n\"", s ) );
}
...
void Cmd_ToggleItem_f( gentity_t *ent )
{
  ...
  else if( BG_InventoryContainsUpgrade( upgrade, ent->client->ps.stats ) )
  {
    ...
    trap_SendServerCommand( ent-g_entities, va( "print \"No tienes el %s\n\"", s ) );
}
...
void Cmd_Buy_f( gentity_t *ent )
{
  ...
  if( buyingEnergyAmmo )
  {
    ...
    {
      trap_SendServerCommand( ent-g_entities, va(
        "print \"Debes estar cerca de un reactor, repetidor o armeria\n\"" ) );
      return;
    }
  }
  else
  {
    ...
    {
      trap_SendServerCommand( ent-g_entities, va( "print \"Tienes que estar cerca de una armeria energizada\n\"" ) );
      return;
    }
  }
  ...
  if( weapon != WP_NONE )
  {
    ...
    if( BG_FindTeamForWeapon( weapon ) != WUT_HUMANS )
    {
      ...
      trap_SendServerCommand( ent-g_entities, va( "print \"No puedes comprar cosas de aliens\n\"" ) );
      return;
    }
    ...
    {
      trap_SendServerCommand( ent-g_entities, va( "print \"No puedes comprar eso\n\"" ) );
      return;
    }
    ...
    else
    {
        ...
        if (BG_InventoryContainsUpgrade( UP_BATTLESUIT, ent->client->ps.stats ))
        {
            ...
            if (BG_InventoryContainsWeapon( WP_CHAINGUN, ent->client->ps.stats ) )
                maxAmmo += BSUIT_AMMOCHAIN;
        }
    }
  ...
  else if( upgrade != UP_NONE )
  {
    ...
    {
      //shouldn't need a fancy dialog
      trap_SendServerCommand( ent-g_entities, va( "print \"No pudes comprar cosas de aliens\n\"" ) );
      return;
    }
  ...
  {
    trap_SendServerCommand( ent-g_entities, va( "print \"No existe lo que quieres comprar\n\"" ) );
  }
...
void Cmd_Sell_f( gentity_t *ent )
{
  ...
  if( weapon != WP_NONE )
  {
    ...
    if( !BG_FindPurchasableForWeapon( weapon ) )
    {
      trap_SendServerCommand( ent-g_entities, va( "print \"Ese arma no se puede vender\n\"" ) );
      return;
    }
    ...
    if( BG_InventoryContainsWeapon( weapon, ent->client->ps.stats ) )
    {
      ...
      if( ( weapon == WP_HBUILD || weapon == WP_HBUILD2 ) &&
          ent->client->ps.stats[ STAT_MISC ] > 0 )
      {
        trap_SendServerCommand( ent-g_entities, va( "print \"No puedes venderlo hasta que se expire el tiempo de construccion\n\"" ) );
        return;
      }
  ...
  else if( upgrade != UP_NONE )
  {
    ...
    if( !BG_FindPurchasableForUpgrade( upgrade ) )
    {
      trap_SendServerCommand( ent-g_entities, va( "print \"No puedes vender eso\n\"" ) );
      return;
    }
  ...
  else if( !Q_stricmp( s, "weapons" ) )
  {
    for( i = WP_NONE + 1; i < WP_NUM_WEAPONS; i++ )
    {
      ...
      if( ( i == WP_HBUILD || i == WP_HBUILD2 ) &&
          ent->client->ps.stats[ STAT_MISC ] > 0 )
      {
        trap_SendServerCommand( ent-g_entities, va( "print \"No puedes venderlo hasta que expire el tiempo de construccion\n\"" ) );
        continue;
      }
  ...
  else if( !Q_stricmp( s, "upgrades" ) )
  {
    ...
    trap_SendServerCommand( ent-g_entities, va( "print \"No existe lo que quieres vender\n\"" ) );
...
void Cmd_Build_f( gentity_t *ent )
{
  ...
  {
    trap_SendServerCommand( ent-g_entities,
      "print \"Se te han quitado los derechos de construccion\n\"" );
    return;
  }
        ...
        ( team == PTE_HUMANS && BG_FindStagesForBuildable( buildable, g_humanStage.integer ) ) ) )
  {
    ...
    trap_SendServerCommand( ent-g_entities, va( "print \"No se puede construir eso\n\"" ) );
}
 ...
 */
 void Cmd_Share_f( gentity_t *ent )
 {
   ...
   {
     trap_SendServerCommand( ent-g_entities,
       "print \"Ooh, que generoso eres, indeed!\n\"" );
     return;
   }
   ...
   {
     trap_SendServerCommand( ent-g_entities,
       va( "print \"share: el jugador no puede recibir mas %s.\n\"",
         ( team == PTE_HUMANS ) ? "credits" : "evolvepoints" ) );
     return;
   }
 ...
 */
 ...
 =================
 */
extern qboolean DrawNodes( gentity_t *ent, qboolean clear );
  ...
  qboolean found = qfalse;
	...
	if(G_SayArgc( ) == 2 || G_SayArgc( ) == 3)
	{
		...
		else if(G_SayArgc( ) == 3 && !Q_stricmp( cmd, "action" ))
		{
			...
			{trap_SendServerCommand( ent-g_entities,
				"print \"Unknown action (jump,wallclimb,kneel,pounce,none)\n\"");return;}
			trap_SendServerCommand( ent-g_entities,
		...
		else if( !Q_stricmp( cmd, "add" ) )
		{
			...
			else
			{
				...
				level.paths[level.numPaths].random = 1;
				level.paths[level.numPaths].timeout = 5000;
...
commands_t cmds[ ] = {
  ...
  { "drop", CMD_HUMAN|CMD_LIVING, Cmd_DropItem_f }
};
...
void ClientCommand( int clientNum )
{
  ...
  if( i == numCmds )
  {
    if( !G_admin_cmd_check( ent, qfalse ) )
      trap_SendServerCommand( clientNum,
        va( "print \"Comando desconocido %s\n\"", cmd ) );
    return;
  }
  ...
  {
    trap_SendServerCommand( clientNum, "print \"Primero entra en un equipo\n\"" );
    return;
  }
  ...
  {
    trap_SendServerCommand( clientNum,
      "print \"No puedes usar este comando mientras estes en un equipo\n\"" );
    return;
  }
  ...
  {
    trap_SendServerCommand( clientNum,
      "print \"Debes entrar como alien para usar este comando\n\"" );
    return;
  }
  ...
  {
    trap_SendServerCommand( clientNum,
      "print \"Debes entrar como humano para usar este comando\n\"" );
    return;
  }
  ...
  {
    trap_SendServerCommand( clientNum,
      "print \"Tiene que estar vivo para usar este comando\n\"" );
    return;
  }
