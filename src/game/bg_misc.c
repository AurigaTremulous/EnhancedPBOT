...
================
*/
// Cambiar la gravedad aca afecta a la prediccion del cliente
// porque el cliente siempre usa un valor de 800
...
qboolean BG_WeaponIsFull( weapon_t weapon, int stats[ ], int psAmmo[ ], int psAmmo2[ ] )
{
  ...
  if ( BG_FindUsesEnergyForWeapon ( weapon ) )
  {
	...
	maxAmmo = (int)( (float)maxAmmo * BATTPACK_MODIFIER );
      ...
      if( BG_InventoryContainsUpgrade( UP_BATTLESUIT, stats ) )
  ...
  else
  {
        ...
        if (BG_InventoryContainsUpgrade( UP_BATTLESUIT, stats ))
        {
            if (BG_InventoryContainsWeapon( WP_MACHINEGUN, stats ) )
                maxAmmo += BSUIT_AMMOCHAIN_RIFLE;
            else
            if (BG_InventoryContainsWeapon( WP_CHAINGUN, stats ) )
