...
g_admin_cmd_t g_admin_cmds[ ] = 
  {
    ...
    {"credits", G_admin_credits, "c",
      "give free credits to a player",
      "[^3name|slot#|admin#^7] [^3amount^7]"
    },
...
qboolean G_admin_credits( gentity_t *ent, int skiparg )
{
  ...
  {
    ADMP( va( "^3!credits: ^7sorry, but your intended victim has a higher admin"
              " level than you\n" ) );
    return qfalse;
  }
  vic = &g_entities[ pids[ 0 ] ];
    CPx( pids[ 0 ], "cp \"^1You have received free credits.\"" );
    AP( va(
      "print \"^3!credits: ^7%s^7 received ^7%d^7 free credits by %s^7\n\"",
      vic->client->pers.netname,
      cred,
      ( ent ) ? G_admin_adminPrintName( ent ) : "console" ) );

  G_AddCreditToClient( vic->client, cred, qfalse);
...
//ROTAX
extern void G_BotToggle( int clientNum );
...
qboolean G_admin_bot( gentity_t *ent, int skiparg ) {
	...
	if( G_SayArgc() < minargc )	{
		...
		ADMP( "^3!bot: ^7usage: !bot [add/del/toggle] [name] (team) (skill)\n" );
		return qfalse;
	}
	...
	if(!Q_stricmp(command,"add")) {
		...
		minargc = 5 + skiparg;
		if(G_SayArgc() < minargc) {
			skill_int = 1; // Por defecto no al maximo (que locura y poco natural)
			// Cuanto mas grande skill sea mas lento va a ser el bot para encontrar
			// su enemigo.
		} else {
		...
		return success;
	} else if(!Q_stricmp(command,"toggle")) {
		// player [name]
		success = qfalse;
		// No funciona bien esto: Togglea clientes reales pero produce errores raros.
		// Con los bots parece funcionar bien.
		for( i = 0; i < MAX_ADMIN_NAMELOGS && g_admin_namelog[ i ] && (success != qtrue);i++ ) {
			if( g_admin_namelog[ i ]->slot >= 0 ) {
				for( j = 0; j < MAX_ADMIN_NAMELOG_NAMES && g_admin_namelog[ i ]->name[ j ][ 0 ]; j++ ) {
					G_SanitiseName(g_admin_namelog[ i ]->name[ j ], name2_s);
					if( strstr( name2_s, name_s ) ) {
						G_BotToggle(g_admin_namelog[ i ]->slot);
						success = qtrue;
						break;
					}
				}
...
qboolean DrawNodes( gentity_t *ent, qboolean clear )
{
	...
	// quiso desactivar el modo devdraw.
	// Si no se encontro ninguna entonces buscamos todos los nodos y "spawneamos"
	// las entidades nodedraw.
	if (clear == qtrue)
	{
		...
		target = G_Spawn();
		target->classname = "nodedraw";
		target->s.eType = ET_MISSILE;
		target->r.svFlags = SVF_USE_CURRENT_ORIGIN;
		target->damage = 0;
		target->splashDamage = 0;
		target->splashRadius = 0;
		if (level.humans_timeapaths > level.time && level.humans_apaths[i] > 0)
		    target->s.weapon = WP_LUCIFER_CANNON;
		else
		if (level.aliens_timeapaths > level.time && level.aliens_apaths[i] > 0)
		    target->s.weapon = WP_LOCKBLOB_LAUNCHER;
		else
		    target->s.weapon = WP_BLASTER;
		target->s.generic1 = WPM_PRIMARY;
		target->s.pos.trType = TR_LINEAR;
		target->s.pos.trTime = level.time;
		target->parent = ent;
		target->r.ownerNum = ent->s.number;
		VectorCopy( level.paths[i].coord, target->s.pos.trBase );
		VectorCopy( level.paths[i].coord, target->r.currentOrigin );
		SnapVector( target->s.pos.trDelta );
	    }
	    ADMP( "^7Se muestran los nodos.\n" );
    return qtrue;
}
qboolean G_admin_node( gentity_t *ent, int skiparg ) {
	int minargc;
	...
	minargc = 2 + skiparg;
	if( G_SayArgc() < minargc )	{
		ADMP( "^7Al menos especifica algo.\n" );
		ADMP( "^3!node: ^7uso: !node [devdraw]\n" );
		return qfalse;
	}

	if (g_pathediting.integer <= 0)
	{
	    ADMP( "^7No se puede usar !node si g_pathediting esta desactivado.\n" );
	    return qfalse;
	}
	
	G_SayArgv( 1 + skiparg, command, sizeof( command ) );
	
	if(!Q_stricmp(command,"devdraw")) {
...
qboolean G_admin_ban( gentity_t *ent, int skiparg )
{
  ...
  if( g_admin_namelog[ logmatch ]->slot == -1 ) 
  {
    ...
    AP( va( "print \"^3!ban:^7 %s^7 ha sido censurado por %s^7 "
      "duracion: %s, motivo: %s\n\"",
      g_admin_namelog[ logmatch ]->name[ 0 ],
      ( ent ) ? G_admin_adminPrintName( ent ) : "console",
      duration,
      ( *reason ) ? reason : "censurado por admin" ) );
    return qtrue;
  }
  
  trap_SendServerCommand( g_admin_namelog[ logmatch ]->slot,
    va( "disconnect \"Has sido censurado.\n"
      "admin:\n%s^7\nduracion:\n%s\nmotivo:\n%s\n%s\"",
      ( ent ) ? G_admin_adminPrintName( ent ) : "console",
      duration,
      ( *reason ) ? reason : "censurado por admin", notice ) );

  trap_DropClient(  g_admin_namelog[ logmatch ]->slot,
    va( "censurado por %s^7, duracion: %s, motivo: %s",
      ( ent ) ? G_admin_adminPrintName( ent ) : "console",
      duration,
      ( *reason ) ? reason : "censurado por admin" ) );
  return qtrue;
