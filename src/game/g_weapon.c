...
void meleeAttack( gentity_t *ent, float range, float width, int damage, meansOfDeath_t mod )
{
  ...
  if( traceEnt->takedamage )
    G_Damage( traceEnt, ent, ent, forward, tr.endpos, damage, 0/*DAMAGE_NO_KNOCKBACK*/, mod );
...
void massDriverFire( gentity_t *ent )
{
  ...
  else
  {
    ...
    tent->s.weapon = ent->s.weapon;
    tent->s.generic1 = ent->s.generic1; //weaponMode
    G_RadiusDamage(tent->r.currentOrigin, ent, 
    MDRIVER_SPLASHDMG, MDRIVER_RADIUS, tent, MOD_MDRIVER_SPLASH);
  }
...
void blasterFire( gentity_t *ent )
{
  if ( BG_InventoryContainsUpgrade(UP_BATTLESUIT, ent->client->ps.stats) )
  {
    ent->client->ps.weaponTime = BLASTER_BSUIT_REPEAT;
  }
  else
  if ( BG_InventoryContainsUpgrade(UP_BATTPACK, ent->client->ps.stats) )
  {
    ent->client->ps.weaponTime = BLASTER_BATTPACK_REPEAT;
  }
  else
  {
    ent->client->ps.weaponTime = BLASTER_REPEAT;
  }

  if ( BG_InventoryContainsWeapon(WP_MASS_DRIVER, ent->client->ps.stats) )
  {
    ent->client->ps.weaponTime = (int)(ent->client->ps.weaponTime * BLASTER_MDRIVER_REPEAT);
  }

  if ( BG_InventoryContainsWeapon(WP_LUCIFER_CANNON, ent->client->ps.stats) )
  {
    ent->client->ps.weaponTime = (int)(ent->client->ps.weaponTime * BLASTER_LCANNON_REPEAT);
  }

  if ( BG_InventoryContainsWeapon(WP_LAS_GUN, ent->client->ps.stats) || 
       BG_InventoryContainsWeapon(WP_LUCIFER_CANNON, ent->client->ps.stats) )
  {
    lasGunFire( ent );
  }
  else
...
======================================================================
*/
void painSawFireShock ( gentity_t *ent )
{
  trace_t   tr;
  vec3_t    end;
  ...
  VectorMA( muzzle, PAINSAW_SHOCKRANGE, forward, end );

  G_UnlaggedOn( muzzle, PAINSAW_SHOCKRANGE );
  trap_Trace( &tr, muzzle, NULL, NULL, end, ent->s.number, MASK_SHOT );
  G_UnlaggedOff( );
  ...
  if( traceEnt->takedamage )
  {
    int damage = PAINSAW_SHOCKDMG;
    if( BG_InventoryContainsUpgrade( UP_BATTPACK, ent->client->ps.stats ) )
    {
	damage = (int) ( damage * PAINSAW_BATTPACKM );
    }
    G_Damage( traceEnt, ent, ent, forward, tr.endpos,
    damage, 0, MOD_PAINSAW );
  }
  else
    return;
...
void painSawSearchFire( gentity_t *ent )
{
  // Ahora dispara un rayo
  ...
  gentity_t *enemy;  
    ...
    VectorSet( range, PAINSAW_SHOCKRANGE, PAINSAW_SHOCKRANGE, PAINSAW_SHOCKRANGE );
    ...
    for( i = 0; i < num; i++ )
    {
      ...
      if( enemy->client && enemy->client->ps.stats[ STAT_PTEAM ] == PTE_ALIENS &&
          enemy->health > 0 &&
          Distance( enemy->s.pos.trBase, ent->s.pos.trBase ) <= PAINSAW_SHOCKRANGE )
      {
        VectorSubtract( enemy->s.pos.trBase, ent->s.pos.trBase, dir );
        VectorNormalize( dir );
        vectoangles( dir, ent->turretAim );

        //fire at target
        painSawFireShock( ent );
      }
    }
}
...
void painSawFire( gentity_t *ent )
{
  ...
  if( traceEnt->takedamage )
  {
    int damage = PAINSAW_DAMAGE;
    if( BG_InventoryContainsUpgrade( UP_BATTPACK, ent->client->ps.stats ) )
    {
	damage = (int) ( damage * PAINSAW_BATTPACKM );
    }
    G_Damage( traceEnt, ent, ent, forward, tr.endpos, damage, DAMAGE_NO_KNOCKBACK, MOD_PAINSAW );
  }
  
  /*if (ent->client->time1000 > 1000-PAINSAW_REPEAT)
  {
    painSawSearchFire( ent );
  }*/
}
...
void LCChargeFire( gentity_t *ent, qboolean secondary )
{
  ...
  if( secondary )
  {
    m = fire_luciferCannon( ent, muzzle, forward, LCANNON_SECONDARY_SPEED );
...
void FireWeapon2( gentity_t *ent )
{
  ...
  switch( ent->s.weapon )
  {
    ...
    /*case WP_CHAINGUN:
      bulletFire( ent, CHAINGUN_SEC_SPREAD, CHAINGUN_DMG, MOD_CHAINGUN );
      break;*/
