...
void G_BounceMissile( gentity_t *ent, trace_t *trace )
{
  ...
  // PARA HACER: Que rebote no siempre en una perfecta direccion ya que
  // las superficies en la realidad no son perfectamente planas. O sea,
  // agregar algo de aleatoriedad al plano de rebote.
  if( ent->s.eFlags & EF_BOUNCE_HALF )
  {
    VectorScale( ent->s.pos.trDelta, 0.50, ent->s.pos.trDelta );
    // check for stop
    if( trace->plane.normal[ 2 ] > 0.2 && VectorLength( ent->s.pos.trDelta ) < 44 )
    {
...
Detect presence in proximity and explode
It is used for the new function of the grenade
================
*/
void G_DetectExplode( gentity_t *ent )
{
	int vectorRange = 200;
	//vectorRange = ent->splashRadius; // <- No compila esto y no entiendo por que
	int i;
	int total_entities;
	...
	//G_Printf( S_COLOR_YELLOW "WARNING: Detected: %s\n", target->classname );
	if ( target->client && (target->client->ps.stats[ STAT_PTEAM ] == PTE_ALIENS) && target->health > 0 )
	{
	    trap_Trace( &trace, ent->r.currentOrigin, NULL, NULL, target->s.pos.trBase, ent->s.number, MASK_PLAYERSOLID );
	    traceEnt = &g_entities[ trace.entityNum ];
	    //if( trace.fraction < 1.0 ) {continue;}
	    if (traceEnt != target) { continue; }
...
{
  /* En Invasion la Lucifer Cannon dispara granadas
  ...
  /*
  PARA HACER: Un poco de aletoriedad en los disparos,
  no puede ser tan perfecto!
  */
  /*
  La funcion siguiente no da resultados, hace un desastre depende
  hacia donde apuntemos.
  No entiendo como funciona forward :S
  dir[0] *= (random()/1000);
  dir[1] *= (random()/1000);
  dir[2] *= (random()/1000);*/
  VectorNormalize( dir );
  ...
  bolt->r.ownerNum = self->s.number;
  bolt->parent = self;
  bolt->damage = LCGRENADE_DAMAGE;
  bolt->splashDamage = LCGRENADE_DAMAGE;
  bolt->splashRadius = LCGRENADE_RANGE;
...
void AHive_SearchAndDestroy( gentity_t *self )
{
  ...
  if( tr.entityNum == ENTITYNUM_WORLD ||
      Distance( self->r.currentOrigin, self->parent->r.currentOrigin ) > ( HIVE_RANGE * 2 ) ||
