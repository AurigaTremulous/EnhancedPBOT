...
struct gentity_s
{
  ...
  //int			numOldPaths;
  int			APaths[MAX_PATHS];
  int			i_APaths; // apaths index
  botstate		state;
  int			buytime;
  int			evolvetime;
  qboolean		noammo;
  ...
  // A continuacion cuanto rapido es el bot apuntando.
  // Esto le da un poco de personalidad
  // Si no se especifica se generan aleatoriamente dentro de valores razonables
  // No funciona en aliens si no esta definido ALIENS_NATURAL
  int		botAimSpeed;
  int		botAimSpeedNear;
  int		botAimSpeedPath;
  int		botAimSpeedPathNear;
  
  // Mas personalidad, preferencia por armas
  float		botPrefBattleSuit;
  float		botPrefPainSaw;
  float		botPrefShotgun;
  float		botPrefLasgun;
  float		botPrefMassDriver;
  float		botPrefChaingun;
  float		botPrefPulseRifle;
  float		botPrefFlamer;
  float		botPrefLuciferCannon;
  
  // Preferencia por defender
  // Si es cero, siempre sale a atacar.
  // Si tiene algun valor dependera del valor
  // la tendencia que tenga a quedarse
  float		botPrefDefend;
  
  // Preferencia por seguir a un amigo
  // Si es 0 nunca sigue a un amigo.
  // 1.0f indica que siempre va a seguir a un amigo.
  // Otros valores menores a 1.0f indican probabilidad
  // (en porcentaje: 0.54f = 54%)
  float		botPrefFriend;
};
...
#define RAM_FRAMES  1                       // number of frames to wait before retriggering
  ...
  // Las modificaciones a los bots
  // El bot humano me gustaria que usara algo como un mouse
  // Simulare un mouse entonces:
  int		delta_angles_e[3]; // Delta Angles expected
  //int		bottimePath; // Contador para determinar si alcanzo el path o no.
  int		bottimeTask; // Contador para la tarea que va a hacer  
  int		bottimeFriend;
  int		bottimeRespawn;
  int		bottimeBuild;
};
...
typedef struct
{
  ...
  // Como funciona el "marcado de camino"
  // Un humano va caminando por el mapa muy contento cuando de repente
  // se encuentra con 3 aliens hambrientos dispuestos a comerselo!
  // El humano mientras estaba caminando fue guardando en un registro
  // propio el camino que recorrio. Cuando ve a esos 3 aliens entonces
  // decide que es hora de dar a conocer el camino a sus amigos.
  // Bueno, humans_apaths indica en orden cada uno de los paths que
  // el humano tuvo que recorrer hasta encontrase con los 3 aliens.
  // Ademas, en timeapaths marca el momento en el que notifico su camino,
  // asi otro humano no va a notificar un camino por ver 3 aliens
  // tambien hasta que cierto tiempo no haya pasado desde que se dio
  // nota del camino anterior.
  // Esto sirve para que se mantenga un flanco defensivo de parte de
  // los bots humanos en un area determinada y que tenga cierta constancia.
  // Mientras los bots humanos aguantan la batalla en ese lugar nosotros
  // podemos ir a atacar a otra area y asi acorralar a los aliens.
  // Pero la historia no se limita a los humanos:
  // Un alien de los chiquitos como una arania va recorriendo un camino
  // hasta que se cruza con un telenodo, en ese momento da a conocer
  // su camino asi los demas aliens se dirigen hacia ese telenodo :P
  // Imaginate que gran batalla se va dar!
  int		humans_timeapaths;
  int		humans_apaths[MAX_PATHS];
  int		aliens_timeapaths;
  int		aliens_apaths[MAX_PATHS];
...
gentity_t *fire_luciferCannon( gentity_t *self, vec3_t start, vec3_t dir, int speed );
...
extern  vmCvar_t  g_RandomStage;
extern  vmCvar_t  g_RandomStageHumans;
extern  vmCvar_t  g_RandomStageAliens;
extern  vmCvar_t  g_RandomAmbush;
extern  vmCvar_t  g_RandomDefensesHumans;
extern  vmCvar_t  g_RandomDefensesAliens;
extern  vmCvar_t  g_RandomBaseHumans;
extern  vmCvar_t  g_RandomBaseAliens;

extern  int  ROTACAK_ambush_rebuild_time_temp;
extern  int  ambush_nexttime;
